(function () {
            ...
            .map(
                ...
                <div class="side-cart-item" data-cart-item-id="${item.id}">
                    ...
                    <div class="item-info">
                        ...
                        <div class="text-muted" style="font-size:12px;">Size: ${SIZE_LABELS[item.size_name] || item.size_name || "Vừa"}</div>
    ...
    function upsertGuestItem(productId, quantity, meta) {
        ...
        const sizeName = String(meta?.size || "S").toUpperCase();
        const existing = cart.items.find(
            (item) => Number(item.product_id) === Number(productId) && String(item.size_name || "S").toUpperCase() === sizeName
        );
        ...
        if (existing) {
            ...
            cart.items.push({
                id: `guest-${productId}-${sizeName}`,
                product_id: productId,
                product_name: meta?.name || "Sản phẩm",
                image_url: meta?.image || "",
                size_name: sizeName,
                size_extra: Number(meta?.sizeExtra || 0),
                quantity,
    ...
    async function addToCart(productId, quantity, meta) {
        const sizeName = String(meta?.size || "S").toUpperCase();
        if (!window.isAuthenticated) {
            upsertGuestItem(productId, quantity || 1, {
                ...(meta || {}),
                size: sizeName,
                sizeExtra: Number(meta?.sizeExtra || 0),
            });
            return true;
        }
    ...
    document.addEventListener("click", async function (event) {
        ...
        const itemId = itemEl.dataset.cartItemId || "";
